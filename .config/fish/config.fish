...
if status is-interactive
  ...
  abbr -a -p command dfa 'viddy -p -n 0.1 -c "paste -d \" \" <(df --output=source,iavail,avail) <(df --output=iavail,ipcent,avail,pcent,target -h) | awk \'{printf \"%s\t%s(%s)\t%s\t%s(%s)\t%s\t%s\n\",\$1,\$2,\$4,\$5,\$3,\$6,\$7,\$8;}\' | column -t -s \$\'\\\\t\' | (sed -u 1q; sort -h -r -k 4) # Sort by Avail"'
  abbr -a -p command dfh 'viddy -p -n 0.1 -c "paste -d \" \" <(df --output=source,iavail,avail) <(df --output=iavail,ipcent,avail,pcent,target -h) | awk \'{printf \"%s\t%s(%s)\t%s\t%s(%s)\t%s\t%s\n\",\$1,\$2,\$4,\$5,\$3,\$6,\$7,\$8;}\' | column -t -s \$\'\\\\t\' | (sed -u 1q; sort -h -r -k 5) # Sort by Use%"'
