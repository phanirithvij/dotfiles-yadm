...
if status is-interactive
  ...
  abbr -a -p command dfah 'viddy -p -n 0.1 -c "df --output=source,iavail,ipcent,avail,pcent,target -h | (sed -u 1q; sort -h -r -k 4) # Sort by Avail"'
  abbr -a -p command dffh 'viddy -p -n 0.1 -c "df --output=source,iavail,ipcent,avail,pcent,target -h | (sed -u 1q; sort -h -r -k 5) # Sort by Use%"'
  abbr -a -p command dfao 'viddy -p -n 0.1 -c "df --output=source,iavail,ipcent,avail,pcent,target | (sed -u 1q; sort -h -r -k 4) # Sort by Avail"'
  abbr -a -p command dffo 'viddy -p -n 0.1 -c "df --output=source,iavail,ipcent,avail,pcent,target | (sed -u 1q; sort -h -r -k 5) # Sort by Use%"'
  abbr -a -p command dfa 'viddy -p -n 0.2 -c "paste -d \" \" <(df --output=source,iavail,avail) <(df --output=iavail,ipcent,avail,pcent,target -h) | awk \'{printf \"%s\t%s(%s)\t%s\t%s(%s)\t%s\t%s\n\",\$1,\$2,\$4,\$5,\$3,\$6,\$7,\$8;}\' | column -t -s \$\'\\\\t\' | (sed -u 1q; sort -h -r -k 4) # Sort by Avail"'
  abbr -a -p command dfh 'viddy -p -n 0.2 -c "paste -d \" \" <(df --output=source,iavail,avail) <(df --output=iavail,ipcent,avail,pcent,target -h) | awk \'{printf \"%s\t%s(%s)\t%s\t%s(%s)\t%s\t%s\n\",\$1,\$2,\$4,\$5,\$3,\$6,\$7,\$8;}\' | column -t -s \$\'\\\\t\' | (sed -u 1q; sort -h -r -k 5) # Sort by Use%"'
  abbr -a -p command dfw 'viddy -p -n 0.2 -c "x=\$(paste -d \" \" <(df --output=source,iavail,avail) <(df --output=iavail,ipcent,avail,pcent,target -h) | awk \'{printf \"%s\t%s(%s)\t%s\t%s(%s)\t%s\t%s\n\",\$1,\$2,\$4,\$5,\$3,\$6,\$7,\$8;}\' | column -t -s \$\'\\\\t\'); echo -e \"\t\t\t\t\t\tSort by Avail\n\$x\n\" | (sed -u 2q; sort -h -r -k 4);echo -en \"\t\t\t\t\t\tSort by Use%\n\$x\" | (sed -u 2q; sort -h -r -k 5)"'
  ...
  abbr -a -p command nixfire 'nixGL firefox & disown;tmux splitw;exit'
