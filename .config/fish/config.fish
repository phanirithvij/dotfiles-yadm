...
if status is-interactive
  ...
  abbr -a -p command dfa 'viddy -p -n 0.1 -c "df --output=source,iavail,ipcent,avail,pcent,target | (sed -u 1q; sort -r -k 4) # Sort by Avail"'
  abbr -a -p command dff 'viddy -p -n 0.1 -c "df --output=source,iavail,ipcent,avail,pcent,target | (sed -u 1q; sort -r -k 5) # Sort by Use%"'
