...
local debugp = require("gears.debug")
--#region
...
cq:wrap(function()
    ...
    while true do
        ...
        if message_type == "message" then
            debugp.print_error("Channel: " .. item[2] .. " Message: " .. item[3])
            if item[2] == "quit" then break end
        elseif message_type == "pmessage" then
            debugp.print_error("Channel: " .. item[3] .. " Message: " .. item[4])
        end
    end
end)
...
-- TODO this is duplicated figure out what the line's purpose is
...
-- https://bbs.archlinux.org/viewtopic.php?pid=1243915#p1243915
-- {{{ Random Wallpapers
...
-- Shuffle a table https://stackoverflow.com/a/68486276/8608146
...
changeTime = 10 -- workaround for the very fist wallpaper
...
-- TODO this doesnt work: Apply a random wallpaper on startup.
...
globalkeys =
    ...
    awful.key(
        ...
        "a",
        function()
            awful.util.spawn(terminal .. " -e ncpamixer")
        end,
    ...
    awful.key(
        ...
        "b",
        function()
            awful.util.spawn("surf www.youtube.com/c/DistroTube")
        end,
    ...
    awful.key(
        ...
        "c",
        function()
            awful.util.spawn(terminal .. " -e cmus")
        end,
    ...
    awful.key(
        ...
        "e",
        function()
            awful.util.spawn(terminal .. " -e neomutt")
        end,
    ...
    awful.key(
        ...
        "f",
        function()
            awful.util.spawn(terminal .. " -e sh ./.config/vifm/scripts/vifmrun")
        end,
    ...
    awful.key(
        ...
        function()
            awful.util.spawn(
                terminal .. " -e lynx --cfg=~/.lynx/lynx.cfg --lss=~/.lynx/lynx.lss -vikeys gopher://distro.tube"
            )
        end,
    ...
    awful.key(
        ...
        "m",
        function()
            awful.util.spawn(terminal .. " -e toot curses")
        end,
    ...
    awful.key(
        ...
        "p",
        function()
            awful.util.spawn(terminal .. " -e pianobar")
        end,
    ...
    awful.key(
        ...
        "r",
        function()
            awful.util.spawn(terminal .. " -e rtv")
        end,
    ...
    awful.key(
        ...
        "w",
        function()
            awful.util.spawn(terminal .. " -e wopr report.xml")
        end,
    ...
    awful.key(
        ...
        "y",
        function()
            awful.util.spawn(terminal .. " -e youtube-viewer")
        end,
    ...
    awful.key(
        ...
        "Print",
        function()
            awful.util.spawn("scrot 'D-%Y-%m-%d-%s_screenshot_$wx$h.jpg' -e 'mv $f $$(xdg-user-dir PICTURES)'")
        end,
...
-- changed so that only alacritty's opacity goes down
-- not telegram, browser, other windows, etc.
client.connect_signal(
    "unfocus",
    function(c)
